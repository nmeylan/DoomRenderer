...
(function() {
  var COLORMAP_SIZE, Colormap, DIRECTORY_LUMP_ENTRY_SIZE, LUMP_NAMES, LumpInfo, NUMBER_PALETTES, Palette, Playpal, Texture, ctx;
  ...
  ctx.Wad = (function() {
    ...
    Wad.prototype.readTextures = function() {
      var i, numberTexture, texture, textureLump, textureName, textureOffset, _i, _ref, _results;
      this.textures = {};
      textureLump = this.lumpHash["TEXTURE1"].getByteData(this.data);
      numberTexture = textureLump.getInt32(0);
      _results = [];
      for (i = _i = 0, _ref = numberTexture - 1; _i <= _ref; i = _i += 1) {
        textureOffset = textureLump.getInt32(0x04 + (0x04 * i));
        textureName = textureLump.getString(textureOffset, 8);
        texture = new Texture(textureName, this.data, textureOffset + 0x08);
        _results.push(this.textures[textureName] = texture);
      }
      return _results;
    };
  ...
  Texture = (function() {
    function Texture(name, data, offset) {
      this.name = name;
      this.data = data;
      this.offset = offset;
      this.width = this.data.getInt16(offset + 0x04);
      this.height = this.data.getInt16(offset + 0x06);
      this.xCenter = 0;
      this.yCenter = 0;
    }
